...
export function hideEventNames(genericName) {
  ...
  document
    .querySelectorAll("div.lFe10c > div > div.Jmftzc.gVNoLb.EiZ8Dd")
    .forEach((elem) => {
      console.warn("here");
      if (!genericName) {
        elem.style.color = "transparent";
  ...
  document
    .querySelectorAll("li > div > div.toUqff.qZvm2d-ibnC6b-bN97Pc.HRaT6d")
    .forEach((elem) => {
      if (genericName) {
        elem.innerHTML =
          '<span jsslot="" style="color: var(--on-surface);">Some Calendar</span>';
      } else {
        elem.style.color = "transparent";
      }
    });
